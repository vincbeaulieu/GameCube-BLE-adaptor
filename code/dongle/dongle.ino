...
#include <stdio.h>
#include <conio.h>
#include <windows.h>  // please feel free to port this to linux, thanks :)
#include <mmsystem.h>
...
#pragma pack(push,1)
typedef struct {
  WORD buttons; ///> button state (use GCB_xxx bit masks)
...
bool StartDriver(const char *name) {
  ...
  bool done=false;
  do {
    SC_HANDLE drv = OpenService(scm,name,SERVICE_ALL_ACCESS);
    if (!drv) break;

    done = (StartService(drv,0,NULL) == TRUE);
...
void gc_open() {
  ...
  // setup port state, sleep a moment in case pad sends early
  _outp(PAR_DATA,PD_DATA|PD_NCLK|PD_RST);
  Sleep(1);
...
bool gc_read_sr(GCPadData *padData) {
  ...
  for (long i=0;i<64;i++) {
    ...
    _outp(PAR_DATA, PD_DATA);     // clock=0
    _outp(PAR_DATA, PD_DATA|PD_NCLK); // clock=1
  }
...
int main (int argc, char **argv) {
  ...
  do {
    ...
    // sleep 20ms to avoid querying pad too frequently
    Sleep(20);
  } while (!quit);
